...
<body class="bg-gray-50 min-h-screen">
        ...
        <div class="container mx-auto px-4 py-4">
            <div class="flex justify-between items-center">
                <div class="flex items-center gap-4">
                    <a href="{{ route('booking.movie.detail', $showtime->movie_id) }}"
                        class="text-gray-500 hover:text-purple-600 text-2xl transition-colors">←</a>
                    <h1 class="text-2xl font-bold text-gray-900">🎬 Cinema Service</h1>
                </div>
                <div class="flex gap-4">
                    @auth('web')
                        <a href="{{ route('booking.index') }}" class="text-gray-600 hover:text-purple-600 font-medium">Đặt
                            Vé</a>
                        <a href="{{ route('my.bookings') }}" class="text-gray-600 hover:text-purple-600 font-medium">Vé Của
                            Tôi</a>
                        <a href="{{ route('dashboard') }}" class="text-gray-600 hover:text-purple-600 font-medium">Cài
                            Đặt</a>
                    @else
                        <a href="{{ route('auth.login.form') }}"
                            class="text-gray-600 hover:text-purple-600 font-medium">Đăng nhập</a>
                    @endauth
                </div>
            </div>
        </div>
    ...
    <div class="container mx-auto px-4 py-8">
        <div class="max-w-7xl mx-auto">
                    ...
                    <div>
                        ...
                        <div class="text-gray-600">
                            ...
                            <p class="flex items-center gap-2">
                                ...
                                {{ \Carbon\Carbon::parse($showtime->start_at)->format('H:i, d/m/Y') }}
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-8 mb-8 shadow-md">
                <!-- Screen -->
                <div class="mb-12">
                    <div class="bg-gradient-to-b from-purple-600 to-transparent h-3 rounded-t-3xl mb-2 opacity-50">
                    </div>
                                ...
                                <div class="flex gap-2">
                                    @foreach($seats as $seat)
                                        ...
                                        <div class="{{ $seatClass }}" data-seat-id="{{ $seat->id }}"
                                            data-seat-number="{{ $seat->seat_number }}" data-row="{{ $row }}" @if($isAvailable)
                                            onclick="toggleSeat(this)" @endif>
                                            {{ $seat->seat_number }}
                                        </div>
                                    @endforeach
                                </div>
    ...
    <script>
        ...
        async function toggleSeat(element) {
            ...
            const isSelected = element.classList.contains('bg-purple-600');

            try {
                if (isSelected) {
                    // Unlock seat
                    const response = await fetch('{{ route("seats.unlock") }}', {
                        method: 'DELETE',
                        headers: {
                    ...
                    if (data.success) {
                        ...
                        // Show error
                        alert(data.message || 'Không thể chọn ghế này');
                    }
                }
